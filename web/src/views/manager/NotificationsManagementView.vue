  ...
  <div class="notifications-management-view">
    ...
    <div class="content-section">
      ...
      <div v-else-if="filteredNotifications.length === 0" class="empty-state">
        ...
        <button class="btn btn-primary" @click="openCreateModal">Créer une notification</button>
      </div>
      <div v-else>
            ...
            <div v-for="template in filteredTemplates" :key="template.id" class="template-card">
              <div class="template-header">
                ...
                <div class="template-actions">
                  <button class="btn-icon" @click="editTemplate(template.id)" title="Modifier">
                    <i class="fas fa-edit"></i>
                  </button>
                  <button class="btn-icon" @click="deleteTemplate(template.id)" title="Supprimer">
                    <i class="fas fa-trash"></i>
                  </button>
                </div>
              </div>
              ...
              <div class="template-footer">
                ...
                <button class="btn btn-sm btn-outline" @click="useTemplate(template.id)">
                  Utiliser
                </button>
              </div>
            </div>
        ...
        <div class="detail-section" v-if="selectedNotification.delivery_id">
          ...
          <div class="detail-grid">
            ...
            <div class="detail-item">
              <span class="detail-label">Type</span>
              <span class="detail-value">{{
                selectedNotification.delivery_type || 'Standard'
              }}</span>
            </div>
          </div>
          <button
            class="btn btn-outline mt-3"
            @click="viewDelivery(selectedNotification.delivery_id)"
          >
        ...
        <div class="detail-section" v-if="selectedNotification.status === 'failed'">
          ...
          <div class="error-details">
            <div class="error-message">
              {{ selectedNotification.error_message || 'Erreur inconnue' }}
            </div>
    ...
    >
      <div class="notification-form">
        <div class="form-group">
          ...
          <input type="text" id="notification-title" v-model="newNotification.title" required />
        </div>
        ...
        <div class="form-group">
          ...
          <div
            ...
            :class="{ 'limit-reached': newNotification.content.length > 500 }"
          >
    ...
    >
      <div class="template-form">
        <div class="form-group">
          ...
          <input type="text" id="template-name" v-model="newTemplate.name" required />
        </div>

        <div class="form-group">
          <label for="template-title">Titre <span class="required">*</span></label>
          <input type="text" id="template-title" v-model="newTemplate.title" required />
        </div>
        ...
        <div class="form-group">
          ...
          <div
            class="character-count"
            :class="{ 'limit-reached': newTemplate.content.length > 500 }"
          >
        ...
        <div class="form-group">
          ...
          <div class="variables-list">
            <div class="variable-item" v-for="variable in availableVariables" :key="variable.name">
              ...
              <button class="btn btn-sm btn-outline" @click="insertVariable(variable.name)">
                Insérer
              </button>
            </div>
          </div>
      ...
      <div class="delete-confirmation">
        ...
        <div class="confirmation-message">
          <p>
            ...
            {{ deleteType === 'notification' ? 'cette notification' : 'ce modèle' }} ?
          </p>
    ...
    const filteredNotifications = computed(() => {
      ...
      if (activeTab.value === 'clients') {
        filtered = filtered.filter(
          n => n.target === 'clients' || (n.recipient && n.recipient.role === 'client')
        )
      } else if (activeTab.value === 'couriers') {
        filtered = filtered.filter(
          n => n.target === 'couriers' || (n.recipient && n.recipient.role === 'courier')
        )
    ...
    const isValidNotification = computed(() => {
      ...
      if (!notification.sendNow) {
        ...
        const scheduledDateTime = new Date(
          `${notification.scheduleDate}T${notification.scheduleTime}`
        )
        const now = new Date()
        ...
        notifications.value = [
          {
            ...
            content:
              "Une nouvelle livraison est disponible dans votre zone. Consultez l'application pour plus de détails.",
            type: 'info',
            status: 'sent',
          ...
          },
          {
            ...
            content:
              'Votre livraison #123 est en cours. Le coursier est en route vers votre adresse.',
            type: 'success',
            status: 'read',
          ...
          },
          {
            ...
            content:
              'Nous avons rencontré un problème avec votre dernier paiement. Veuillez vérifier vos informations de paiement.',
            type: 'error',
            status: 'sent',
          ...
          },
          {
            ...
            content:
              "Une maintenance système est prévue le 20 mai de 2h à 4h du matin. L'application pourrait être indisponible pendant cette période.",
            type: 'warning',
            status: 'sent',
          ...
          },
          {
            ...
            content:
              "L'envoi du SMS de confirmation a échoué. Veuillez vérifier le numéro de téléphone du destinataire.",
            type: 'error',
            status: 'failed',
      ...
      try {
        ...
        templates.value = [
          ...
          },
          {
            ...
            content:
              'Votre livraison #{{delivery_id}} est en cours. Le coursier {{courier_name}} est en route vers votre adresse.',
            type: 'success',
            target: 'clients',
            channel: 'in_app',
          },
          ...
          },
          {
            ...
            title: 'Maintenance système prévue',
            content:
              "Une maintenance système est prévue le {{date}} de {{start_time}} à {{end_time}}. L'application pourrait être indisponible pendant cette période.",
            type: 'warning',
            target: 'all',
            channel: 'in_app',
          },
