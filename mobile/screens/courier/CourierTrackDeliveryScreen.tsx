...
interface CourierTrackDeliveryScreenProps {
  route: {
    params: {
      deliveryId: number
    }
...
const CourierTrackDeliveryScreen = ({ route, navigation }: CourierTrackDeliveryScreenProps) => {
  const { deliveryId } = route.params
  const { user } = useAuth()
  const [delivery, setDelivery] = useState<Delivery | null>(null)
    ...
    try {
      ...
      switch (newStatus) {
        case 'pickup_in_progress':
          updatedDelivery = await DeliveryService.startPickup(delivery.id)
          break
        case 'picked_up':
          updatedDelivery = await DeliveryService.confirmPickup(delivery.id)
          break
        case 'in_transit':
          updatedDelivery = await DeliveryService.startDelivery(delivery.id)
          break
        case 'delivered':
          updatedDelivery = await DeliveryService.confirmDelivery(delivery.id)
          break
        default:
          return
      }
  ...
  const getNextAction = () => {
    ...
    switch (delivery.status) {
      case 'accepted':
        return {
          text: 'Commencer la collecte',
          action: () => handleStatusUpdate('pickup_in_progress'),
  ...
  const getCurrentDestination = () => {
    ...
    if (['accepted', 'pickup_in_progress'].includes(delivery.status)) {
      return {
      ...
      >
          ...
          <View style={styles.actionSection}>
            ...
            >
              {actionLoading ? (
                ...
                <>
                  <Ionicons name={nextAction.icon as any} size={24} color="#ffffff" />
                  <Text style={styles.primaryActionText}>{nextAction.text}</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
            ...
            <View style={styles.destinationCard}>
              <View style={styles.destinationInfo}>
                ...
                <View style={styles.destinationDetails}>
                  ...
                  {destination.contact && (
                    <Text style={styles.destinationContact}>Contact: {destination.contact}</Text>
                  )}
                </View>
              </View>
              ...
              <View style={styles.destinationActions}>
                {destination.phone && (
                  <TouchableOpacity
                    ...
                    onPress={() => handleCall(destination.phone!)}
                  >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => handleOpenMaps(destination.address, destination.lat, destination.lng)}
                >
        ...
        <View style={styles.mapSection}>
          <View style={styles.mapContainer}>
            <VTCStyleMap
              initialRegion={{
                latitude: destination?.lat || 5.3599517,
                longitude: destination?.lng || -4.0282563,
                latitudeDelta: 0.01,
                longitudeDelta: 0.01,
              }}
              deliveries={[delivery]}
        ...
        <View style={styles.deliveryInfoSection}>
          ...
          <View style={styles.infoCard}>
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoValue}>{delivery.client?.full_name || 'N/A'}</Text>
            </View>
            
            <View style={styles.infoRow}>
              <Text style={styles.infoLabel}>Prix:</Text>
              <Text style={styles.infoPrice}>
                {delivery.final_price || delivery.proposed_price} FCFA
              </Text>
            </View>
            ...
            <View style={styles.infoRow}>
              <Text style={styles.infoLabel}>Distance estimée:</Text>
              <Text style={styles.infoValue}>{delivery.estimated_distance || 0} km</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.secondaryActionsSection}>
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('DeliveryChat', { deliveryId: delivery.id })}
          >
            <Ionicons name="chatbubble-outline" size={20} color="#007AFF" />
            <Text style={styles.secondaryActionText}>Contacter le client</Text>
          </TouchableOpacity>
          ...
          >
            ...
            <Text style={styles.secondaryActionText}>Signaler un problème</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
